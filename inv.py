...
PASSWORD = os.environ.get("SP_PASSWORD", "tu_password_aqui")
...
def obtener_mapa_fotos(ctx, folder_url):
    """Obtiene todos los archivos de la carpeta para mapearlos por su nombre sin extensión."""
    print("📸 Mapeando carpeta de fotos...")
    try:
        folder = ctx.web.get_folder_by_server_relative_url(folder_url)
        files = folder.files.get().execute_query()
        # Diccionario: {'A5767': '/sites/.../A5767.jpg'}
...
def descargar_y_comprimir_foto(ctx, relative_url):
    """Descarga la foto y la convierte a base64 para embeberla en el HTML."""
    try:
...
def main():
    try:
        ...
        for index, row in df.iterrows():
            ...
            if not cod_interno: continue # Saltar filas sin código
            
            cod_foto = str(row.get('Código Fotografía asociada', '')).strip()
            img_b64 = None
            
            # Si hay código de foto y existe en la carpeta, la descargamos
...
# ==========================================
# 4. GENERADOR HTML (DASHBOARD INVENTARIO)
# ==========================================
...
<head>
    ...
    <style>
        ...
        * { box-sizing: border-box; }
        ...
        .f-group label { display: block; font-size: 0.8rem; font-weight: 700; color: var(--secondary); margin-bottom: 5px; }
...
<body>
    ...
    <script>
        ...
        function buildFilters() {
            ...
            let html = createSelect('f_cat', 'Categoría / Familia', getUnique('categoria'));
            html += createSelect('f_planta', 'Planta Asociada', getUnique('planta'));
            html += createSelect('f_crit', 'Criticidad', getUnique('criticidad'));
        ...
        function applyFilters() {
            ...
            const filtered = records.filter(d => {
                ...
                if (search) {
                    const txt = `${d.nombre} ${d.codigo_sap} ${d.ubicacion_fisica}`.toLowerCase();
            ...
            data.forEach(d => {
                ...
                card.innerHTML = `
                    ...
                    <div class="card-body">
                        ...
                        <p class="c-info">📦 <b>Stock/Repetido:</b> ${d.stock || '0'}</p>
                    </div>
                `;
        ...
        function openModal(d) {
            ...
            const imgHtml = d.img_base64 
                ...
                : `<div style="color:#94a3b8; font-style:italic;">📷 Sin fotografía asociada</div>`;
